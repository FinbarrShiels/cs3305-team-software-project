...
function Theheader() {
    ...
    const user = useUser() // The user object from UserContext
    const userLogOut = useUserLogOut() // Logs the current user out from firebase
        ...
        <header>
            <nav>
                ...
                <div className="menuItems">
                    ...
                    {/* The next two elements are rendered if there is no current user */}
                    ...
                    {/* The next two elements are rendered if there is a valid current user */}
                    {user !== null && <li><NavLink className="logOutButton" to="/login" onClick={() => userLogOut()}> Log Out </NavLink></li>}
                    {user !== null && user.fname && user.sname && <li><NavLink className="userBadge" to="/profile"> {`${user.fname.charAt(0)} ${user.sname.charAt(0)}`} </NavLink></li>}
                </div>
